 ...
 *
 * 1. Caching the pizza container divs and the total number of these divs. This was done by moving
 * the call to `document.querySelectorAll(".randomPizzaContainer");` out of the `for` loop,
 * therefore the DOM tree was traversed once an not for each iteration of the for loop.
 *
 * 2. Caching the new width of the pizza container divs. All the `.randomPizzaContainer` elements
...
function drawMovingPizzas() {
  ...
  var s = 256;
  var height = 100;
  // Calculate the height of the screen and create required number of pizzas to animate
  var rows = Math.floor(window.innerHeight/height) ;
  var numMovingPizzas = cols * rows;

  // Add movingPizzas1 div via javascript. This is done in order to easily redraw the moving
  // pizzas when window is resized.
