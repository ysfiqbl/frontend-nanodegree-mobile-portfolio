...
/**
 * Optimized resizePizzas function to reduced the resize pizza functionality to less than 5 ms by
 * 1. Replaced querySelector and querySelectorAll function calls by getElementById and
 * getElementsByClassName functions.
 *
 * 2. Caching the pizza container divs and the total number of these divs. This was done by moving
 * the call to fetch the pizzas out of the `for` loop,
 * therefore the DOM tree was traversed once and not for each iteration of the for loop.
 *
 * 3. Caching the new width of the pizza container divs. All the `.randomPizzaContainer` elements
 * have the same layout width, this width is `33.33%` and is defined in the `pizzaElementGenerator`
 * function. Therefore the new layout width for all the elements will also be the same. Hence the
 * call to the `determineDx` function can also be moved out the `for` loop.
 */
var resizePizzas = function(size) {
	...
	function changePizzaSizes(size) {
		...
		var pizzaContainersLength = allPizzaContainers.length;
 ...
 * 2. Decoupled scroll event from animation, see line 520.
 * 3. Calculates phase outside the for loop. The i%5 values in the for loop had a
 * pattern. They values were a repeating series of 0, 1, 2, 3 and 4. Therefore the
 * phase calculation could be extracted outside the loop. Also document.body.scrollTop
 * is unique for each scroll event and therefore it was also extracted outside the
 * for loop.
 */
...
/**
 * Abstracted moving pizzas creation and drawing to its own function.
 * The number of moving pizzas draw on the screen now depends on the window
 * size. The height of each pizza image is 100px, therefore the number of
 * rows that pizzas needs to be drawn on are calculated by dividing the window
 * height by 100 and rounding it off to the lowest int value. The number of
 * pizzas is obtained by multiplying the number of rows with the number of columns.
 *
 * movingPizzas1 div is now added via javascript. This makes it easier and more
 * efficient to redraw the moving pizzas when the window is resized.
 *
 * This function also gets called when the window is resized. When the window is resized
 * after a DOMContentLoaded event, the DOM will contain a movingPizzas1 div.
 * Therefore if a movingPizzas1 div exists, it is removed and recreated along with
 * its moving pizza children in order to draw the correct number of moving pizzas
 * for the new window size.
 *
 * Optimizations:
 * 1. Replaced querySelector and querySelectorAll function calls by getElementById and
 * getElementsByClassName functions.
 * 2. Cached moving pizza elements in a global variable so it does not need to be fetched
 * everytime a scroll executes the updatePositions function.
 */
function drawMovingPizzas() {
	var cols = 8;
	var s = 256;
	var height = 100;
	...
	if (movingPizzas1Div != null) {
		headerDiv.removeChild(movingPizzas1Div);
	}
	movingPizzas1Div = document.createElement("div");
	movingPizzas1Div.id = "movingPizzas1";
	movingPizzas1Div.className = "col-md-6";
	...
	document.getElementById("header").appendChild(movingPizzas1Div);
	window.items = document.getElementsByClassName("mover");
}

// runs requestTick function on scroll
...
// Redraw moving pizzas
window.onresize = drawMovingPizzas;
